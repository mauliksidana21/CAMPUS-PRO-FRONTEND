...
export default function Home() {
  ...
  async function doLogin() {
    ...
    try {
      ...
      if (data.authenticated) {
        ...
        setError("Invalid credentials. Email aur password check karo.");
      }
    } catch (e) {
      setError("Server error. Goscraper running hai? CMD mein go run . karo");
    }
    setLoading(false);
  }
  ...
  function parseAttendance(data) {
    ...
    return data.attendance.map((a) => ({
      ...
      courseTitle: a.courseTitle || a.courseName || "",
  ...
  function parseMarks(data) {
    ...
    return data.marks.map((m) => ({
      ...
      courseName: m.courseName || m.courseCode || "",
  ...
  async function loadAllData(token) {
    setDataLoading(true);
    try {
      const [attData, marksData, ttData, courseData] = await Promise.all([
        fetchData("attendance", token),
        fetchData("marks", token),
        fetchData("timetable", token),
        fetchData("courses", token),
      ]);
      setAttendance(parseAttendance(attData));
      setMarks(parseMarks(marksData));
      setTimetable(parseTimetable(ttData));
      if (courseData?.courses) setCourses(courseData.courses);
      else setCourses(parseAttendance(attData).map((a) => ({
  ...
  function doLogout() {
    localStorage.clear();
    setEmail(""); setPassword(""); setAttendance([]);
    setMarks([]); setTimetable([]); setCourses([]);
    setPage("login");
  }
    ...
    return (
      <div style={S.loginBg}>
        <div style={S.loginCard}>
          ...
          <label style={S.fieldLabel}>Student Email</label>
          <input
            style={S.inputField}
          ...
          <div style={{ marginTop: 16, fontSize: 11, color: "#8a9bc4", textAlign: "center" }}>
    ...
    <div style={S.appWrap}>
      ...
      <div style={S.sidebar}>
        ...
        <div style={{ margin: "14px 18px 10px", background: "#111d35", border: "1px solid #1e2d4d", borderRadius: 10, padding: "10px 12px", display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, background: "#1a6cf0", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 600, color: "#fff", flexShrink: 0 }}>
            {userEmail.slice(0, 2).toUpperCase()}
          </div>
          <div>
            <div style={{ fontSize: 12, fontWeight: 500, color: "#e8edf8", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 130 }}>{userEmail}</div>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
          ...
          <div style={S.content}>
            <div style={{ background: "#0d1f3c", border: "1px solid #1a6cf0", borderRadius: 16, padding: "20px 24px", marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{ fontSize: 12, color: "#8a9bc4" }}>
            ...
            {attendance.length === 0 ? (
              <div style={S.emptyBox}>{dataLoading ? "Loading..." : "No data — goscraper running hai?"}</div>
            ) : (
              attendance.map((a, i) => {
                ...
                return (
                  <div key={i} style={S.attRow}>
                    ...
                    <div style={{ fontSize: 22, fontWeight: 700, color, minWidth: 70, textAlign: "right" }}>{pct.toFixed(1)}%</div>
                  </div>
                );
              })
            )}
        ...
        {activeNav === "timetable" && (
          <div style={S.content}>
            ...
            <div style={S.pageSub}>Your weekly class schedule</div>
            {timetable.length === 0 ? (
                  ...
                  <div key={i} style={{ background: "#0e1726", border: "1px solid #1e2d4d", borderRadius: 12, padding: "12px 10px" }}>
                    ...
                    {(day.classes || []).map((c, j) => (
                      <div key={j} style={{ background: "#111d35", borderLeft: "3px solid #1a6cf0", borderRadius: 6, padding: "7px 8px", marginBottom: 5 }}>
                        ...
                        <div style={{ fontSize: 10, color: "#5b9bff" }}>{c.startTime && c.endTime ? `${c.startTime} – ${c.endTime}` : c.roomNo || ""}</div>
                      </div>
                    ))}
                  </div>
                ))}
        ...
        {activeNav === "attendance" && (
          <div style={S.content}>
            ...
            <div style={S.pageSub}>Real-time attendance from SRM Academia</div>
            <div style={S.statGrid}>
              {[
                ...
                { label: "Total Subjects", val: attendance.length, color: "#f5c842" },
              ].map((c, i) => (
            ...
            {attendance.length === 0 ? (
              <div style={S.emptyBox}>{dataLoading ? "Loading..." : "No attendance data"}</div>
            ) : (
              attendance.map((a, i) => {
                const pct = parseFloat(a.attendancePercentage);
                const color = pct >= 75 ? "#00c97a" : pct >= 65 ? "#f5c842" : "#e8192c";
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                {marks.map((m, i) => (
                  <div key={i} style={{ background: "#0e1726", border: "1px solid #1e2d4d", borderRadius: 14, padding: 18 }}>
                    <div style={S.attSubject}>{m.courseName}</div>
              ...
              courses.map((c, i) => (
                <div key={i} style={{ background: "#0e1726", border: "1px solid #1e2d4d", borderRadius: 14, padding: "18px 20px", marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
                    ...
                    <div style={{ background: "#0d1f3c", color: "#5b9bff", fontSize: 11, padding: "3px 10px", borderRadius: 20 }}>
                      {c.credit || c.credits || 3} Credits
                    </div>
                  </div>
                  {c.facultyName && (
...
function GPACalc({ subjects }) {
  ...
  return (
    <div style={S.content}>
      ...
      <div style={S.pageSub}>Grade change karo — GPA live update hoga</div>
          ...
          <thead>
            <tr style={{ background: "#111d35" }}>
              {["Subject", "Credits", "Grade", "Points"].map(h => (
                <th key={h} style={{ padding: "10px 14px", fontSize: 11, color: "#8a9bc4", textAlign: "left", textTransform: "uppercase", letterSpacing: 0.5 }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {list.map((c, i) => (
              <tr key={i} style={{ borderBottom: "1px solid #1e2d4d" }}>
                ...
                <td style={{ padding: "10px 14px" }}>
                  <input type="number" min="1" max="6" defaultValue="3" style={S.gpaInput} onChange={e => setCredits({ ...credits, [i]: e.target.value })} />
                </td>
                <td style={{ padding: "10px 14px" }}>
                  <select style={S.gpaInput} value={grades[i] || "A"} onChange={e => setGrades({ ...grades, [i]: e.target.value })}>
      ...
      <div style={{ background: "#0d1f3c", border: "1px solid #1a6cf0", borderRadius: 16, padding: 24, textAlign: "center" }}>
        ...
        <div style={{ fontSize: 13, color: "#8a9bc4" }}>
          {gpa >= 9 ? "First Class with Distinction 🏆" : gpa >= 7.5 ? "First Class 🎉" : gpa >= 6 ? "Second Class" : "Pass Class"}
        </div>
      </div>
    </div>
  );
    ...
    <div style={S.content}>
      ...
      <div style={{ background: "#0e1726", border: "1px solid #1e2d4d", borderRadius: 16, padding: 20, marginBottom: 16 }}>
        <div style={{ marginBottom: 14 }}>
          <div style={S.statLabel}>Subject select karo</div>
          <select style={{ ...S.gpaInput, width: "100%", padding: "10px 12px" }} onChange={e => setSel(parseInt(e.target.value))}>
            {attendance.length > 0
              ? attendance.map((a, i) => <option key={i} value={i}>{a.courseTitle} ({a.attendancePercentage}%)</option>)
              : <option>Pehle login karo</option>}
          </select>
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 14 }}>
          <div>
            <div style={S.statLabel}>Remaining classes (expected)</div>
            <input type="number" style={{ ...S.gpaInput, width: "100%" }} value={rem} onChange={e => setRem(parseInt(e.target.value) || 0)} />
          </div>
          <div>
            <div style={S.statLabel}>Classes to miss</div>
            <input type="number" style={{ ...S.gpaInput, width: "100%" }} value={hol} onChange={e => setHol(parseInt(e.target.value) || 0)} />
          </div>
        </div>
        <button style={{ background: "#e8192c", border: "none", borderRadius: 10, padding: "10px 24px", color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer" }} onClick={calculate}>
          Calculate →
        </button>
      </div>
      {result && (
        <div style={{ background: "#0e1726", border: `1px solid ${result.safe ? "#00c97a" : "#e8192c"}`, borderRadius: 16, padding: 20 }}>
          ...
          <div style={{ padding: "10px 14px", borderRadius: 8, fontSize: 13, background: result.safe ? "#001f10" : "#1f0508", color: result.safe ? "#00c97a" : "#e8192c" }}>
            {result.safe
              ? `✓ Safe! ${result.fp}% attendance hogi. ${result.margin} classes ka margin bachega.`
              : `⚠ Danger! ${Math.abs(result.margin)} aur classes attend karni hongi!`}
          </div>
        </div>
      )}
    </div>
...
const S = {
  ...
  sidebar: { width: 220, background: "#0d1528", borderRight: "1px solid #1e2d4d", display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh", overflowY: "auto" },
  ...
  logoutBtn: { margin: 10, padding: 9, background: "transparent", border: "1px solid #1e2d4d", borderRadius: 8, color: "#8a9bc4", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 },
  content: { padding: 28 },
